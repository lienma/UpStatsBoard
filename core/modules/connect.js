...
function Connection (options) {
	...
	this.os = options.os ? options.os : 'linux';

	var tempOs = (this.os === 'mac') ? 'linux' : this.os;
	if(Connect[tempOs] === undefined) {
		throw new Error('Unsuported operating system.')
	}

	this.func = Connect[this.os];
	...
	return new Promise(function (resolve, reject) {
		if(self.remote === true) {
			if(self._isConnected === true) {
				...
				self.func.connect(this).then(resolve).catch(reject);
			}
		} else {
			resolve();
		}	
...
Connect.linux = {
	connect: function (self) {
		return new Promise(function (resolve, reject) {
			...
			var client = new ssh(), socket = new Socket(), resolved = false;
...
Connect.windows2 = {
	connect: function (self) {
		return new Promise(function (resolve, reject) {
			...
			client.connect(params).then(function(prompt) {
				console.log(prompt);
			}, function(error) {
