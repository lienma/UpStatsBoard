...
var log 	= require(paths.logger)('COUCHPOTATO');

function CouchPotato(config) {
...
CouchPotato.prototype.getPage = function(cmd, args) {
	var url = this.url + 'api/' + this.apiKey + '/' + cmd + '/?' + qs.stringify(args);
	log.debug('Calling CouchPotato page:', chalk.cyan(url));
...
CouchPotato.prototype.ping = function() {
	return this.getPage('app.available');
};
